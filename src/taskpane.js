...
async function getInvoiceSummary(context, invoiceSheet) {
  const usedRange = invoiceSheet.getUsedRange();
  const clientNameRange = invoiceSheet.getRange(CELLS.clientName);
  const emailRange = invoiceSheet.getRange(CELLS.clientEmail);
  const invoiceNumberRange = invoiceSheet.getRange(CELLS.draftStatus);

  usedRange.load("values");
  clientNameRange.load("values");
  emailRange.load("values");
  invoiceNumberRange.load("values");
  await context.sync();

  const rows = usedRange.values;
  return {
    invoiceNumber: invoiceNumberRange.values[0][0],
    clientName: clientNameRange.values[0][0],
    email: emailRange.values[0][0],
    total: getCellValueFromMatrix(rows, "Total"),
    dueDate: getCellValueFromMatrix(rows, "Date Due"),
    issueDate: getCellValueFromMatrix(rows, "Date of Issue"),
  };
}
